...
/// Casper validator set maintained in a contract, updated using `getValidators` method.
...
use_contract!(validator_set, "CasperSet", "res/contracts/casper_set.json");
...
struct CasperValidators {
    validators: HashMap<Address, U256>,
}

impl HeapSizeOf for CasperValidators {
    fn heap_size_of_children(&self) -> usize {
        self.validators.heap_size_of_children()
    }
}

impl CasperValidators {
    fn contains(&self, _: &H256, address: &Address) -> bool {
        self.validators.contains_key(address)
    }

    /// Draws a validator nonce modulo number of validators.
    fn get(&self, parent: &H256, nonce: usize) -> (Address, U256) {
        ...
        let key = self.validators.keys().nth(nonce % validator_n).expect("There are validator_n authorities; taking number modulo validator_n gives number in validator_n range; qed");

        let val = self.validators.get(key).unwrap();
        (key.clone(), val.clone())
    }

    /// Returns the current number of validators.
    fn count(&self, _: &H256) -> usize {
        self.validators.len()
    }
}
...
//    header: Header,
//    provider: validator_set::CasperSet,
//}
//
...
pub struct ValidatorCasperContract {
    contract_address: Address,
    validators: RwLock<MemoryLruCache<H256, CasperValidators>>,
    provider: validator_set::CasperSet,
...
//fn check_first_proof(machine: &EthereumMachine, provider: &validator_set::CasperSet, contract_address: Address, old_header: Header, state_items: &[DBValue])
...
//// of `getValidators`.
//fn prove_initial(provider: &validator_set::CasperSet, contract_address: Address, header: &Header, caller: &Call) -> Result<Vec<u8>, String> {
...
impl ValidatorCasperContract {
    pub fn new(contract_address: Address) -> Self {
        ValidatorCasperContract {
            contract_address,
            validators: RwLock::new(MemoryLruCache::new(MEMOIZE_CAPACITY)),
            provider: validator_set::CasperSet::default(),
            client: RwLock::new(None),
        }
    }

    /// Queries the state and gets the set of validators.
    fn get_list(&self, caller: &Call) -> Option<CasperValidators> {
        ...
        match self.provider.functions().get_validators().call(&caller) {
            Ok(new) => {
                debug!(target: "engine", "Set of casper validators obtained: {:?}", new);
                //Some(SimpleList::new(new))
                None // TODO(joel): fixme
            },
            Err(s) => {
                debug!(target: "engine", "Set of casper validators could not be updated: {}", s);
                None
            },
        }
    }
    ...
    //        None => None,
    //        Some(matched_event) => None, // TODO(joel): fixme Some(SimpleList::new(matched_event.new_set))
    //    }
    //}
}

impl ValidatorSet for ValidatorCasperContract {
    ...
    fn is_epoch_end(&self, first: bool, _chain_head: &Header) -> Option<Vec<u8>> {
        match first {
            true => Some(Vec::new()), // allow transition to casper set, and instantly
            false => None,
        }
    }
        ...
        -> Result<(SimpleList, Option<H256>), ::error::Error>
    {
        ...
        if first {
            trace!(target: "engine", "Recovering initial epoch set at {}", number);
        ...
        //        number, addresses.len());
            ...
            Ok((SimpleList::new(Vec::new()), None))
        } else {
            trace!(target: "engine", "epoch set at {} with proof {:?}", number, proof);
...
//    use super::{ValidatorCasperContract, EVENT_NAME_HASH};
//    use verification::queue::kind::blocks::Unverified;
//
//    #[test]
//    fn fetches_validators() {
//        let client = generate_dummy_client_with_spec_and_accounts(Spec::new_validator_safe_contract, None);
//        let vc = Arc::new(ValidatorCasperContract::new("0000000000000000000000000000000000000005".parse::<Address>().unwrap()));
