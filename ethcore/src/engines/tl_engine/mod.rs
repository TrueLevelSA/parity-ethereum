...
pub struct TLEngine {
    ...
    /// last unix timestamp when a block was signed by this validator 
    last_sign_time: Duration,
}
...
const ENGINE_TIMEOUT_TOKEN: TimerToken = 42;
//TODO: pass as config arg in config file
const MILLIS_BETWEEN_BLOCKS: u64 = 10_000;

impl IoHandler<()> for TransitionHandler {
    fn initialize(&self, io: &IoContext<()>) {
        let random01: u64 = 0;//thread_rng().gen_range(0,1000);
        let timeout_duration = Duration::from_millis(MILLIS_BETWEEN_BLOCKS+random01);
        io.register_timer_once(ENGINE_TIMEOUT_TOKEN, timeout_duration)
            .unwrap_or_else(|e| warn!(target: "engine", "Failed to start consensus step timer: {}.", e))
    }

    fn timeout(&self, io: &IoContext<()>, timer: TimerToken) {
        if timer == ENGINE_TIMEOUT_TOKEN {
            // ask the client to create a new block
        ...
        let engine = Arc::new(
            TLEngine {
                ...
                sender_state: RwLock::new(
                    ...
                    last_sign_time: unix_now().sub(Duration::from_millis(MILLIS_BETWEEN_BLOCKS)),
            });

        //FIXME: should not timeout during test
#[cfg(not(test))]
        {
...
impl Engine<EthereumMachine> for TLEngine {
    ...
    fn verify_block_family(&self, _header: &Header, _parent: &Header) -> Result<(), <EthereumMachine as ::parity_machine::Machine>::Error> {
    ...
    /// operation may not available, in which None will be returned
    fn generate_seal(&self, block: &<EthereumMachine as ::parity_machine::Machine>::LiveBlock, _parent: &Header) -> Seal {
       // if self.last_sign_time > unix_now().sub(Duration::from_millis(MILLIS_BETWEEN_BLOCKS)) {
       //     Seal::None
       // } else {

            let generated = self.sign(block.header().bare_hash())
         ...
         //   if generated != Seal::None {
         //       self.last_sign_time = unix_now();
         //   }
            generated
        //}
    }
